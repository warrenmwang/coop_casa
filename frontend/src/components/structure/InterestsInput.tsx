...
const InterestsInput : React.FC<InputInterestsArgs> = ({ formData, setFormData, setError, setIsChanged, required }) => {
  ...
  const handleInterestsChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    if (checked) {
      ...
      updatedInterests = updatedInterests.length === 0 ? value : `${updatedInterests},${value}`
    } else {
  ...
  return(
    <div className="min-h-full w-full px-3">
      <label
        ...
        htmlFor="interests"
      >
        Interests (Multiple Answer, Select at least 1) {required && <span className="text-red-500">*</span>}
      </label>
